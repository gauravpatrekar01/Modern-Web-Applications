...
# Direct Google Form POST submission endpoint
...
print(f"Loaded {len(df)} rows from Excel. Beginning submission...\n")
...
for idx, row in df.iterrows():
    ...
    if response.status_code == 200:
        print(f"Row {idx + 2} ({row['Full Name']}): Submitted successfully.")
    else:
        print(f"Row {idx + 2} ({row['Full Name']}): Failed with status code {response.status_code}")
